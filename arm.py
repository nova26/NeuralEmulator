...
from NeuralEmulator.Arm.OzNeruModel import OZNeuralModel
...
class Simulation:
    ...
    def __init__(self, model,  # Instance of the mechanical model of the arm
                 ...
                 simulation=None,
                 viewer=None,
                 useOzNetwork=False):  # Number of actuated gripping points

        self.results = results
        self.useOzNetwork = useOzNetwork
        ...
        self.simulation = mjc.MjSim(self.model.mjc_model) if simulation is None else simulation
        self.viewer = mjc.MjViewer(self.simulation) if viewer is None else viewer
        ...
        if adapt:
            ...
            self.ozModel = OZNeuralModel(means=[
                0.12, 2.14, 1.87, 4.32, 0.59,
                0.12, -0.38, -0.42, -0.29, 0.36],
                variances=[
                    0.08, 0.6, 0.7, 0.3, 0.6,
            ...
            trainMean = [-0.005052, -5.808082, -3.221717, 0.262583, 0.064206]
            trainStd = [1.922263, 2.769751, 1.566649, 0.098603, 0.652208]

            self.ozModel.setTrainingSignalFactors(trainMean, trainStd)
    ...
    def simulate(self, steps=None):  # Number of maximum allowable steps for mitigating the target
        ...
        for exp in self.monitor_dict:
            ...
            while True:
                ...
                if self.adaptation:
                    ...
                    if self.useOzNetwork:
                        out = self.ozModel.generate(input_signal=input_signal, training_signal=training_signal)
                        out = out + [0]
                        out = np.asarray(out)
                        u += out
                    else:
                ...
                #   print("Grip {} \ntarget {}".format(ee_position,target[:3]))
    ...
    def show_monitor(self):
        ...
        for exp in self.monitor_dict:
            ...
            self.results.pushX(ee_x)
            self.results.pushY(ee_y)
            self.results.pushZ(ee_z)

        #     ax.plot(ee_x, ee_y, ee_z)
        #
        #     targetX = self.monitor_dict[exp]['target_real'][0]
        #     targetY = self.monitor_dict[exp]['target_real'][1]
        #     targetZ = self.monitor_dict[exp]['target_real'][2]
        #
        #     ax.scatter(targetX, targetY, targetZ, label="target", c="r")
        #     ax.legend()
        #
        # plt.show()
